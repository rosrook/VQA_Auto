...
class BenchmarkManager:
    ...
    def __init__(self, 
        ...
        # 仅加载benchmark分类文件，不加载benchmark数据
        if taxonomy_file:
            self._load_taxonomy_file(taxonomy_file)
        else:
            # 默认路径
    ...
    def load_benchmark(self, bench_name: str, load_data: bool = False) -> Optional[BaseBenchmark]:
        """
        按需加载指定的benchmark（延迟加载，不加载数据）
        
        Args:
            bench_name: benchmark名称
            load_data: 是否立即加载数据（False表示仅创建实例，不加载数据）
        
        Returns:
            Benchmark实例，如果不存在则返回None
        """
        # 如果已经加载过，直接返回
        ...
        # 从catalog获取配置
        if bench_name not in self.benchmark_catalog:
            return None
        
        bench_info = self.benchmark_catalog[bench_name]
        ...
        if bench_info.get("source") == "huggingface":
            try:
                benchmark = HuggingFaceBenchmark(
                    ...
                    split=bench_info.get("default_split", "validation"),
                    benchmark_info=bench_info,
                    load_data_on_init=load_data  # 控制是否立即加载数据
                )
                self.benchmarks[bench_name] = benchmark
                return benchmark
            except Exception as e:
    ...
    def select_benchmark_names(self,
                               requirements: Dict[str, Any] = None,
                               benchmark_names: List[str] = None) -> List[str]:
        """
        选择benchmark名称列表（不加载数据）
        
        Args:
            requirements: 需求字典
            benchmark_names: 指定要使用的benchmark名称列表
        ...
        if benchmark_names:
            # 验证名称是否在catalog中
            valid_names = [name for name in benchmark_names if name in self.benchmark_catalog]
            invalid_names = [name for name in benchmark_names if name not in self.benchmark_catalog]
            if invalid_names:
                print(f"警告: 以下benchmark不存在: {', '.join(invalid_names)}")
            return valid_names
        
        if requirements is None:
            return list(self.benchmark_catalog.keys())
        
        selected = []
        for name, bench_info in self.benchmark_catalog.items():
            # 根据需求筛选（基于catalog信息，不需要加载benchmark）
            if "taxonomy_fields" in requirements:
                native_fields = bench_info.get("native_taxonomy_fields", [])
                required_fields = requirements["taxonomy_fields"]
                if not any(f in native_fields for f in required_fields):
                    continue
